...
class ProductRepositoryImpl @Inject constructor(
    ...
    override fun getProducts(limit: Int, orderBy: String, sort: String): Flow<PagingData<Product>> {
        return getPager(limit) { page, pageSize ->
            apiService.getProduct(page = page, limit = pageSize, orderBy = orderBy, sort = sort)
        }.map { pagingData ->
            pagingData.map { dto -> dto.mapToProduct() }
        }.combine(
            flow { emit(getFavoriteProductIds()) } ) { pagingData, favoriteProductIds ->
    ...
    override fun getFavoriteProducts(limit: Int, orderBy: String, sort: String): Flow<PagingData<Product>> {
        return getPager(limit) { page, pageSize ->
            apiService.getFavoriteProducts(page = page, limit = pageSize, orderBy = orderBy, sort = sort)
        }.map { pagingData ->
            pagingData.map { dto -> dto.mapToProduct() }
        }
    }
    ...
    private suspend fun getFavoriteProductIds(): Set<Int> {
        val response = apiService.getFavoriteProducts(page = 1, limit = 1000, orderBy = "id", sort = "asc")
    ...
    override fun addProductToFavorites(addFavoriteProductRequest: AddFavoriteProductRequest): Flow<Resource<Unit>> =
        flow {
            val response = safeApiCall { apiService.addProductToFavorites(addFavoriteProductRequest) }
            emit(response)
        }
}